...
package com.tezov.lib_core_android_kotlin.animation
        ...
        with(step) {
            ...
            updateTransition(this, label = "").also { transition ->
                animators.forEach { entry ->
                    ...
                    val transition:Transition<Any> = when(animator){
                        is AnimatorChild<*,*> -> {
                        ...
                        is Animator<*> -> transition as Transition<Any>
                        else -> throw IllegalStateException("Unknown animator class")
                    }
                    animator.update(transition = transition)
                }
            }
        }
        LaunchedEffect(key) {
